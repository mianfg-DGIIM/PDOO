...
class SpaceStation
	...
	def initialize(_name, _supplies)
		...
		# EXAMEN
		# !@attribute [Boolean] Determines if the penalty factor when firing is
		#                       used or not (this makes this penalty effective
	...
	# EXAMEN
	# Makes a shot
	# @param factor [Float] initial shot factor: used for penalty
	#                       (if @penalty, fire is multiplied by initial factor)
	# @return [Float] the shot power
	def fire(factor)
		puts "[EXAMEN] Ejecutado fire en SpaceStation con factor=#{factor}, @penalty=#{@penalty}"
		if @penalty == false
			factor = 1.0
		end
